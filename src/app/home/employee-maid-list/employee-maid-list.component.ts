...
@Component({
  selector: 'app-employee-maid-list',
  templateUrl: './employee-maid-list.component.html',
  styleUrls: ['./employee-maid-list.component.scss'],
})
export class EmployeeMaidListComponent implements OnInit {
    ...
    this._crud.get_emp_list().subscribe(
      (res: any) => {
        if (res && res.AllRegisteredEmployee) {
          this.reg_data = res.AllRegisteredEmployee.filter((employee: any) => employee.employee_type === "Maid");
